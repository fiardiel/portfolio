        ...
        <div className="w-full lg:w-[1024px] md:px-6 flex justify-between items-center">
          <div className="w-full md:w-auto">
            <Button
              onPress={scrollToTop}
              disableRipple
              className="text-2xl font-semibold p-0 bg-transparent gap-1"
            >
              ...
              <div key={index} className="w-min h-min">
                <Button
                  ...
                  onPress={
                    item.name === "Home"
                      ? scrollToTop
                      : () => scrollToElementById(item.path)
                  }
                  className={`text-lg p-0 bg-transparent min-w-0 min-h-0 rounded-none underline-animation underline-animation-blue ${isActive(item.path) ? "text-primary" : ""}`}
                >
                  <span>{item.name}</span>
                </Button>
              </div>
